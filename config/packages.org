...
(use-package counsel-projectile
  ...
  (:map projectile-command-map
        ...
        ("d" . projectile-discover-project-in-search-path)) ;; C-c p d
  :config
...
(use-package csv-mode
  ...
  :config
  ;; Mantener líneas largas sin wrap para facilitar lectura tabular.
...
(use-package conda
  ...
  :config
  (let* ((env-home (getenv "CONDA_HOME"))
         (conda-home (if (and env-home (not (string-equal env-home "")))
                         (expand-file-name env-home)
                       (expand-file-name "~/miniconda3"))))
    (setq conda-anaconda-home conda-home
          conda-env-home-directory conda-home))
...
#+begin_src emacs-lisp
  (use-package tramp
    ...
    (setq tramp-default-method "ssh"
          tramp-verbose 1)
    :config
    ;; Heredar el PATH remoto para que herramientas como python, julia, etc. se encuentren correctamente.
    (add-to-list 'tramp-remote-path 'tramp-own-remote-path))

  (if (locate-library "tramp-container")
  ...
  ;; Reusar agentes SSH existentes (útil para repos y servidores remotos)
  (use-package ssh-agency
    :ensure t
    :after tramp
    :config
    (when (require 'ssh-agency nil 'noerror)
      (if (fboundp 'ssh-agency-mode)
          (ssh-agency-mode 1)
        (message "ssh-agency cargado; usa M-x ssh-agency-add-key si es necesario."))))
#+end_src
